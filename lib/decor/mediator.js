...
(function() {
  var Mediator, bindable, bindableCall, comerr, step, type,
    __slice = [].slice;
  ...
  Mediator = (function() {
    ...
    */
    ...
    Mediator.prototype.execute = function(command, context, options, next) {
      ...
      next = type(args[args.length - 1]) === "function" ? args.pop() : function() {};
      context.loading = true;
      request = bindableCall(context, function(next) {
        var callbacks, listener;

        args.push(next);
        if (listener = _this._listeners[command]) {
          callbacks = listener.pre.concat(listener.callback || []).concat(listener.post);
          return step.call(request, args, callbacks);
        } else {
          return next(comerr.notFound("command '" + command + "' not found."));
        }
      });
      request.bind("response").once().to(function(response) {
